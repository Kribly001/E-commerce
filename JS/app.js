  let namevisit = prompt("Ingresa tu nombre");
  while (namevisit === "") {
      alert("No ingreso su nombre");
      namevisit = prompt("Ingresa tu nombre");
  }
  alert("Bienvenido " + namevisit + " disfruta tu estadia aqui, para saber el precio del producto presiona la imagen");

  function saberprecio(number) {
      let valor;
      let compra = prompt("Su precio es $" + number + " desea agregar al carrito? SI o NO");
      if (compra == "SI") {
          alert("Compra finalizada pronto le llegara por correo")
      } else if (compra == "NO") {
          alert("No se completo la compra")
      } else {
          alert("No se ingreso un valor validoo");
      }
      return compra;
  }



let product = document.getElementsByClassName("nameproduct")
console.log(product[0].innerHTML);
console.log(product[1].innerHTML);
console.log(product[2].innerHTML);
console.log(product[3].innerHTML);
console.log(product[4].innerHTML);
console.log(product[5].innerHTML);